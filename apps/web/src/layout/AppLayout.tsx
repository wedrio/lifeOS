...
const menuItems: MenuProps['items'] = [
  navigation[0],
  {
    ...
    label: '自我生长',
    children: [navigation[1], navigation[2]],
  },
  {
    key: 'finance',
    icon: <WalletOutlined />,
    label: '财务脉络',
    children: [navigation[3], navigation[4], navigation[5]],
  },
  {
    key: 'life',
    icon: <AppstoreOutlined />,
    label: '生活档案',
    children: [navigation[6], navigation[7]],
  },
  navigation[8],
];
...
const mobileNavigation = [
  ...
  { ...navigation[3], label: '财务', section: 'finance' },
  { ...navigation[6], label: '生活', section: 'life' },
  { ...navigation[8], label: '设置', section: 'settings' },
];
...
function NavigationMenu({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return <Menu
    ...
    onClick={({ key }) => { navigate(String(key)); onNavigate?.(); }}
  />;
}
...
function MobileNavigation() {
  ...
  return <nav className="mobile-nav" aria-label="移动端主导航">{mobileNavigation.map((item) => <NavLink className={currentSection === item.section ? 'active' : undefined} key={item.key} to={item.key} end={item.key === '/'}>{item.icon}<span>{item.label}</span></NavLink>)}</nav>;
}
    ...
    <Layout className="page-layout">
      <Header className="topbar">
        <div className="topbar-actions">
          ...
          <span className="topbar-title"><i /><span className="topbar-section">{sectionLabel(currentSection)}</span><em>/</em><strong>{currentItem.label}</strong></span>
        </div>
        <div className="topbar-actions">
