...
function CategoryModal({ category, categories, onClose, onSaved }: { category?: Category; categories: Category[]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  useEffect(() => { form.setFieldsValue({ name: category?.name, icon: category?.icon ?? '🏷️', color: category?.color ?? '#5b5ce2', type: category?.type ?? 'expense', parentId: category?.parentId }); }, [category, form]);
  ...
  return <Modal open title={category ? '编辑分类' : '新建分类'} onCancel={onClose} footer={null} destroyOnClose><Form form={form} layout="vertical" onFinish={submit} initialValues={{ icon: '🏷️', color: '#5b5ce2', type: 'expense' }}><Form.Item name="type" label="类型" rules={[{ required: true }]}><Segmented block options={[{ label: '支出', value: 'expense' }, { label: '收入', value: 'income' }]} onChange={() => form.setFieldValue('parentId', undefined)} /></Form.Item><Form.Item name="name" label="名称" rules={[{ required: true, message: '请输入分类名称' }]}><Input maxLength={40} placeholder="例如：咖啡" /></Form.Item><Space.Compact block><Form.Item name="icon" label="图标" style={{ width: '34%' }} rules={[{ required: true }]}><Input maxLength={8} /></Form.Item><Form.Item name="color" label="颜色" style={{ width: '66%' }} rules={[{ required: true }]}><Input type="color" style={{ height: 32 }} /></Form.Item></Space.Compact><Form.Item name="parentId" label="父分类（可选）"><Select allowClear placeholder="不设置父分类" options={categories.filter((item) => item.type === type && item.id !== category?.id).map((item) => ({ value: item.id, label: `${item.icon} ${item.name}` }))} /></Form.Item><div className="modal-footer"><Button onClick={onClose}>取消</Button><Button type="primary" htmlType="submit">保存</Button></div></Form></Modal>;
}
