...
export function MomentsPage() {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const [nextMoments, habits, transactions, assets] = await Promise.all([
        dataSource.moments.list(), dataSource.habits.list(), dataSource.finance.listTransactions(), dataSource.assets.list(),
      ]);
  ...
  return <>
    ...
    {loading ? <Card loading style={{ minHeight: 260 }} /> : view === 'timeline' ? <Timeline moments={moments} linkLabels={linkLabels} onEdit={openEditor} onDelete={remove} /> : <CalendarView month={month} selectedDate={selectedDate} momentsByDate={momentsByDate} onSelect={setSelectedDate} linkLabels={linkLabels} onEdit={openEditor} />}
